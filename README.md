...
- **Recovery Guidance** — Replaces universal recovery countdowns with versioned context from logged modeled history: latest load, seven-day frequency and workload, athlete-relative baseline comparison, confidence and coverage. It is not a recovery or readiness measurement. [Guidance contract](RECOVERY_GUIDANCE.md).
- **Movement Assessment (prototype)** — A separate, camera-based prototype for one bodyweight squat filmed side-on. A pose model that ships with the app runs on the device and produces video-estimated 2D mechanics: apparent knee range of motion, knee and trunk–thigh angle traces, descent/ascent/total time, and capture quality. Left/right symmetry is not estimated from a single side view. It does not measure force, tissue load, injury risk or recovery, is not connected to Tissue Load or Recovery Guidance, and saves nothing — video, frames and results are never stored or uploaded. [Method, privacy and limitations](MOVEMENT_ASSESSMENT.md) · [Verification](MILESTONE_6_VERIFICATION.md).
