...
export const EXERCISES = EXERCISES_BY_CATEGORY;

// Evidence-backed range for hypertrophy is ~10–20 sets/week per muscle.
// Default to the lower end so most lifters can realistically hit it.
export const DEFAULT_WEEKLY_SET_TARGET = 10;
...
export const CARDIO_WEEKLY_DAYS_TARGET = 2;
...
export const EXERCISE_MUSCLE = (function() {
  const map = {};
  EXERCISES_BY_CATEGORY.chest.forEach(function(n)      { map[n] = "chest"; });
  EXERCISES_BY_CATEGORY.back.forEach(function(n)       { map[n] = "back"; });
  EXERCISES_BY_CATEGORY.shoulders.forEach(function(n)  { map[n] = "shoulders"; });
  EXERCISES_BY_CATEGORY.biceps.forEach(function(n)     { map[n] = "biceps"; });
  EXERCISES_BY_CATEGORY.triceps.forEach(function(n)    { map[n] = "triceps"; });
  EXERCISES_BY_CATEGORY.core.forEach(function(n)       { map[n] = "core"; });
  EXERCISES_BY_CATEGORY.cardio.forEach(function(n)     { map[n] = "cardio"; });
  EXERCISES_BY_CATEGORY.glutes.forEach(function(n)     { map[n] = "glutes"; });
  EXERCISES_BY_CATEGORY.quads.forEach(function(n)      { map[n] = "quads"; });
  EXERCISES_BY_CATEGORY.hamstrings.forEach(function(n) { map[n] = "hamstrings"; });
  EXERCISES_BY_CATEGORY.calves.forEach(function(n)     { map[n] = "calves"; });
  return map;
})();
